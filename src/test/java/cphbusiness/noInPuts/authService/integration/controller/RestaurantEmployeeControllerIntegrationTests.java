...
public class RestaurantEmployeeControllerIntegrationTests {
    ...
    public void loginShouldReturnWithID() throws Exception {
        ...
        this.mockMvc.perform(post("/api/auth/restaurantEmployee/login").content("{ \"username\": \"employee_user\", \"password\": \"Password1!\" }").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).characterEncoding("UTF-8"))
    ...
    public void loginShouldReturnBadRequestWhenUsernameIsBlank() throws Exception {
        ...
        this.mockMvc.perform(post("/api/auth/restaurantEmployee/login").content("{ \"username\": \"\", \"password\": \"Password1!\" }").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).characterEncoding("UTF-8"))
    ...
    public void loginShouldReturnUnsupportedMediaTypeWhenNotParsingJson() throws Exception {
        ...
        this.mockMvc.perform(post("/api/auth/restaurantEmployee/login").content("not json").characterEncoding("UTF-8"))
    ...
    public void loginShouldReturnBadRequestWhenCredentialsAreWrong() throws Exception {
        ...
        this.mockMvc.perform(post("/api/auth/restaurantEmployee/login").content("{ \"username\": \"employee_user\", \"password\": \"Password2!\" }").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).characterEncoding("UTF-8"))
                .andExpect(status().isBadRequest());
    }
}
