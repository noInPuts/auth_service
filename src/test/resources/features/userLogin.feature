Feature: Logging in which returns the user object and a JWT Token
  ...
  Scenario: User logging in
    ...
    When I make a login POST request to "/api/user/login" with the following body:
      | username | password |
  ...
  Scenario: User logging in with incorrect password
    ...
    When I make a login POST request with wrong password to "/api/user/login" with the following body:
      | username | password |
